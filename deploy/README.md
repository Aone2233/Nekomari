...
| `tender-guard.service` | The NOSLA Tokyo node. Runs as a non-root `komari` user with `AmbientCapabilities=CAP_NET_RAW` (a *file* capability would be defeated by `NoNewPrivileges`/`PrivateTmp`), and carries its own node token rather than `--auto-discovery`. |
