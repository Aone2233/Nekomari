...
| `install-node-agent.sh` | Installs the agent on a node: refuses to run if one is already active, verifies the download against `SHA256SUMS.txt`, retires older agent units, writes a systemd unit. Works as root or through passwordless sudo. It accepts `-t <token>` (what the panel hands out) but **moves the token into `<install-dir>/.agent-credentials` (0600) and starts the agent with `--token-file`**, so the node's identity is not in `ExecStart`, `ps` or `systemctl show`. Use `--install-token-file PATH` to place the credential elsewhere; a re-run with no token reuses the file an earlier install wrote. `--dry-run` writes the credential and prints the command without downloading or touching the service. |
| `install-node-agent.test.sh` | Proves the above with the real script: an offline harness confining every write to a temp directory, asserting that each spelling of the token argument (`-t v`, `--token=v`) ends up in a mode-0600 file and never in the unit's `ExecStart`, that a re-run reuses the stored credential, and that a missing identity still fails instead of starting a node with none. Runs in CI as the `deploy-scripts` job. |
...
| `panel-probe.sh` | The always-on version of the check above: every 10 minutes it records the origin TTFB (the only number that implicates the panel, so it has the tight threshold) plus the public TTFB with the `cf-ray` POP and cache status, one line per run. A sample only counts as healthy when curl succeeded, the status is 200 and the body is a valid success envelope — a fast 500 or a refused connection exits nonzero and names the reason instead of looking fast. Installed with `hosts/panel-probe.{service,timer}`. `docs/PERFORMANCE.md` explains how to read it. |
| `panel-probe.test.sh` | Proves `panel-probe.sh` can actually fail: local listeners serve a 200 envelope, a 200 with an HTML or `{"status":"error"}` body, a 500 and a deliberately slow response, and the assertions pin the exit code and the named reason for each, plus a refused connection and a zero `time_starttransfer`. Offline and deterministic; `PROBE_LOG` points at a temp file, so it never touches `/var/log`. |
